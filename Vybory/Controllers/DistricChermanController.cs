...
{
    public class DistricChermanController : Controller
    {
        IUnitOfWork db;

        public DistricChermanController(IUnitOfWork unitOfWork)
        {
            db = unitOfWork;
        }
        ...
        public IActionResult DistricChermanCitizen()
        {
            var citizens = db.Citizens.Find(c => c.StatusId == 3);
            if (citizens == null)
            {
                return NotFound();
            }
        ...
        public IActionResult DistricChermanAppeal()
        {
            var citizens = db.Citizens.Find(c => c.StatusId == 3);
            IEnumerable<Appeal> appeals = new List<Appeal>();
            foreach (var citizen in citizens)
            {
                appeals = db.Appeals.Find(a => a.AppealTypeId == 5 || a.CitizenId == citizen.Id);
            }
            if (appeals == null)
            {
                return NotFound();
