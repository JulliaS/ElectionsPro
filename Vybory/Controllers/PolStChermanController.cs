...
{
    public class PolStChermanController : Controller
    {
        IUnitOfWork db;

        public PolStChermanController(IUnitOfWork unitOfWork)
        {
            db = unitOfWork;
        }
        ...
        public IActionResult PolStChermanCitizen()
        {
            var citizens = db.Citizens.Find(c => c.StatusId == 1 || c.StatusId == 4 || c.StatusId == 6);
            if (citizens == null)
            {
                return NotFound();
            }
        ...
        public IActionResult PolStChermanAppeal()
        {
            var citizens = db.Citizens.Find(c => c.StatusId == 1 || c.StatusId == 4 || c.StatusId == 6);
            IEnumerable<Appeal> appeals = new List<Appeal>();
            foreach (var citizen in citizens)
            {
                appeals = db.Appeals.Find(a => a.AppealTypeId == 4 || a.CitizenId == citizen.Id);
            }
            if (appeals == null)
            {
                return NotFound();
